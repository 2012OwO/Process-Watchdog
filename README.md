...
- **500ms 高频轮询**：以 500ms 间隔检测程序A是否退出，实测从检测到退出到拉起程序仅几毫秒
...
- **零依赖**：Release 中的 EXE 已打包全部运行时，下载即用，无需安装 Python 或任何环境

## 快速开始

1. 从 [Releases](../../releases) 下载 `ProcessWatchdog.exe`，放到任意目录
2. 同目录下创建 `config.ini`（首次运行会自动生成模板），按需修改：
...
本项目代码由 AI 助手（WorkBuddy）辅助开发生成，并经人工实测验证（进程退出检测、并发拉起、日志记录均通过真实测试）。
